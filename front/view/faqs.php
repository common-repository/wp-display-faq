...
if ( $WfpData->have_posts() ) {
  ...
  <div class="wfp-main-wrapper">
    ...
    while ( $WfpData->have_posts() ) {
        $WfpData->the_post();
        ?>
      <button class="wfp-collapsible <?php 
        echo  ( 'all' === $wfp_expand_collapse_item ? 'active' : null ) ;
        ?>">
        <?php 
        
        if ( 'yes' === $wfp_display_open_close_icon ) {
            ?>
          <span class="wfp_open_cl_icon fa fa-<?php 
            esc_attr_e( $wdf_open_close_icon );
            ?>"></span>
          <?php 
        }
      ...
      <div class="wfp-content <?php 
        echo  ( 1 === $dfC && 'first' === $wfp_expand_collapse_item ? 'active-first' : null ) ;
        ?>" <?php 
        echo  ( 'all' === $wfp_expand_collapse_item ? 'style="max-height: max-content!important;"' : null ) ;
        ?> data-anim-type="<?php 
        esc_attr_e( $wdf_content_animation );
        ?>">
        <?php 
        the_content();
        ?>
  ...
  <?php 
    ...
    ?>
  <p class="wfp-no-afaqs-found"><?php 
    _e( 'No FAQ Found.', WFP_TXT_DOMAIN );
    ?></p>
  <?php 
}
